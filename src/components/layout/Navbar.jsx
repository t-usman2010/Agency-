  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-white/90 backdrop-blur-xl shadow-sm border-b border-dark-100/50'
          : 'bg-transparent'
      }`}
    >
      <nav className="container-max flex items-center justify-between px-4 sm:px-6 lg:px-8 h-16 sm:h-20">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2 group" aria-label={AGENCY_NAME}>
          ...
          <span className="font-bold text-lg sm:text-xl text-dark-900 hidden sm:block">
            {AGENCY_NAME}
          </span>
        </Link>
        ...
        >
          {isOpen ? (
            <HiXMark className="w-6 h-6 text-dark-900" />
          ) : (
            <HiBars3 className="w-6 h-6 text-dark-900" />
          )}
        </button>
      </nav>
